...
if status is-interactive
    ...
    # Theme: 来自 fisher 的 rose-pine/fish；未安装时跳过（见 fish_plugins，安装：fisher install）
    if type -q fish_config
        set -l _themes (fish_config theme list 2>/dev/null)
        if contains "Rosé Pine" $_themes
    ...
    # 来自 fisher 的 patrickf1/fzf.fish；未安装时跳过（fisher install）
